...
import { Button, Col, Collapse, DatePicker, Divider, Form, Input, InputNumber, Row, Select, TreeSelect } from 'antd';
...
function Filter({ ...props }) {
  ...
  function renderFilterText(data) {
    return <Input placeholder={data.label} disabled={loading} allowClear/>;
  }

  function renderFilterNumber(data) {
    return <InputNumber placeholder={data.label} disabled={loading} allowClear style={{ width: '100%' }}/>;
  }

  function renderFilterSelect(data) {
    return <Select
      placeholder={`${t('CHON')} ${data.label}`} disabled={loading} dropdownClassName="small"
  ...
  function renderFilterMultiSelect(data) {
    return <Select placeholder={`${t('CHON')} ${data.label}`} disabled={loading} dropdownClassName="small"
  ...
  function renderFilterDate(data) {
    return <DatePicker size="small" style={{ width: '100%' }} format="DD-MM-YYYY"
                       placeholder={`${t('CHON')} ${data.label}`} disabled={loading}
  ...
  return (
    ...
    >
      <Collapse.Panel
        showArrow={false}
        className="p-0"
        header={<Button type="primary" size="small" className="mb-2" onClick={() => setCollapse(!isCollapse)}>{t('BO_LOC')} {isCollapse ? <CaretUpOutlined/> : <CaretDownOutlined/>}</Button>}
        key="1">
        <Form form={formFilter} id="form-filter" autoComplete="off" size="small" colon={false} layout="vertical"
...
Filter.defaultProps = {
  layoutCol: { xs: 24, sm: 24, md: 24, lg: 10, xl: 8, xxl: 6 },
  labelCol: { xs: 24, sm: 6, md: 6, lg: 10, xl: 10, xxl: 6 },
